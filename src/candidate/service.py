...
def analyse_candidate(cv_content):
    ...
    # Initialize the Gemini 2.0 model (Google Generative AI)
    llm = ChatGoogleGenerativeAI(model=candidate_config.MODEL_NAME, temperature=0.5)
    
    # Pass the SystemMessage and HumanMessage to the model
    completion = llm.predict_messages(
        [
            SystemMessage(content=system_prompt_candidate),
            HumanMessage(content=cv_content),
        ],
        functions=fn_candidate_analysis,  # Include function handlers if supported
    )
